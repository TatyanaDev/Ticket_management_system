    ...
    <Table responsive>
      <thead>
        <tr className={style.head}>
          <th className='ps-3 pe-3'>Ticket Name</th>
          <th className='pe-3'>Ticket description</th>
          <th className='pe-3'>Ticket Status</th>
      ...
      <tbody>
        {tickets.map((ticket) => (
          <tr key={ticket.id} className={cn("align-middle", style.body)}>
            <td className='pe-3 ps-3'>{ticket.name}</td>
            <td className='pe-3'>{ticket.description}</td>
            <td className='pe-3'>
              <Row className={cn(styleSelection(ticket.status), "m-0")} style={{ fontSize: 14, liheHeight: 24 }}>
                {ticket.status}
              </Row>
            </td>
            <td className='text-nowrap pe-3 pt-1 pb-1'>
